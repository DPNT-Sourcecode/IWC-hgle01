...
def test_priority_resolution_with_dependencies_and_three_tasks_and_different_timestamps() -> None:
    run_queue([
        ...
        call_dequeue().expect("bank_statements", 2),  # time-sensitive (10 min old), t+0 < t+10
        call_dequeue().expect("companies_house", 1),
        call_dequeue().expect("credit_check", 1),
        call_dequeue().expect("id_verification", 1),
    ])
...
def test_priority_duplicated_tasks() -> None:
    run_queue([
        ...
        call_dequeue().expect("bank_statements", 1),  # time-sensitive (5 min old), t+0 < t+5
        call_dequeue().expect("id_verification", 1),
    ])
...
def test_get_age_of_queue(delta_minutes, expected_age) -> None:
    run_queue([
        call_enqueue("bank_statements", 1, iso_ts(delta_minutes=0)).expect(1),
...
def test_get_age_empty_queue() -> None:
    run_queue([
        call_age().expect(0),
    ])
...
def test_priority_bank_statement_2() -> None:
    run_queue([
        ...
        call_dequeue().expect("bank_statements", 1),
        call_dequeue().expect("companies_house", 1), # time-sensitive (6 min old), t+0 < t+6
