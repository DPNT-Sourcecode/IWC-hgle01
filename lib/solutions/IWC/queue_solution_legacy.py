...
class Queue:
    ...
    def dequeue(self):
        ...
        for user_id in user_ids:
            ...
            non_bank_statements_tasks = [t for t in user_tasks if t.provider != "bank_statements"]
            tasks_for_reference = non_bank_statements_tasks  if non_bank_statements_tasks else user_tasks
            earliest_timestamp = sorted(tasks_for_reference, key=lambda t: t.timestamp)[0].timestamp
            priority_timestamps[user_id] = earliest_timestamp
            task_count[user_id] = len(user_tasks)

        for task in self._queue:
            metadata = task.metadata
            is_bank_statement = task.provider == "bank_statements"
        ...
        breakpoint()
        older_bank_statements = [t for t in self._queue if t.provider == "bank_statements" and (latest_timestamp - self._timestamp_for_task(t.timestamp)).total_seconds() >= BANK_STATEMENTS_AGE_THRESHOLD.total_seconds()]
